...
CharacterSelector::CharacterSelector(Engine *engine)
{
    ...
    this ->_ecsManager->addComponent(bg_perso2, std::make_unique<Placable>(start_pos + (400*1), 200));
    this->_ecsManager->addComponent(bg_perso2, std::make_unique<DrawableSprite>(this->_box_texture, 1));
    
    this ->_ecsManager->addComponent(bg_perso3, std::make_unique<Placable>(start_pos + (400*2), 200));
    this->_ecsManager->addComponent(bg_perso3, std::make_unique<DrawableSprite>(this->_box_texture, 1));
    
    this->_ecsManager->addComponent(bg_perso4, std::make_unique<Placable>(start_pos + (400*3), 200));
...
//mcg
    ...
    this->_ecsManager->addComponent(character_mcg, std::make_unique<Placable>(0.0f, 0.0f, 0.0f, rotationAxis, -90.0f, scale));
